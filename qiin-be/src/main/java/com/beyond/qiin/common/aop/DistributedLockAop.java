...
public class DistributedLockAop {
  ...
  public Object lock(final ProceedingJoinPoint joinPoint) throws Throwable { //aspect 대상 메서드
    ...
    log.info("[DISTRIBUTED-LOCK-AOP] Executing lock logic. method={}, key={}",
        method.getName(), key);

    try {
      //lock 접근 시도
      boolean available = rLock.tryLock(distributedLock.waitTime(), distributedLock.leaseTime(), distributedLock.timeUnit());
      log.info("[DISTRIBUTED-LOCK-AOP] method={} key={} -> lock acquired? {}",
          method.getName(), key, available);
      if (!available) {
        return false;
      }
      ...
      try{ 
        rLock.unlock();
        log.info("[DISTRIBUTED-LOCK-AOP] method={} key={} -> UNLOCKED",
            method.getName(), key);
