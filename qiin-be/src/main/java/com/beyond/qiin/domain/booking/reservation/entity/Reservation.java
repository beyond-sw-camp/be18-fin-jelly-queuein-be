...
public class Reservation extends BaseEntity {
    ...
    public void approve(User respondent, String reason) {
        if(this.status != 0)
            throw new IllegalArgumentException("only pending to approved");
        this.status = 1;
    ...
    public void reject(User respondent, String reason) {
        if(this.status != 0)
            throw new IllegalArgumentException("only pending to rejected");
        this.status = 3;
        this.reason = reason; //사용자 입력이므로 null 받으면 null임(빈칸은 프론트에서 ""으로 옴)
        this.respondent = respondent;
    }
    ...
    public void start(){
        if(this.status != 1)
            throw new IllegalArgumentException("only approved to start");
        this.status = 2;
    ...
    public void end(){
        if(this.status != 2)
            throw new IllegalArgumentException("only started to end");
        this.status = 5;
