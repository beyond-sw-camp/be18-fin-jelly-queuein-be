...
public class UserReservationsQueryAdapterImpl implements  UserReservationsQueryAdapter {
  ...
  public Page<GetUserReservationResponseDto> search(
    ...
    // 사용자 예약 필터
    builder.and(reservation.applicant.getId.eq(userId));
    ...
    if (condition.getReservationStatus() != null) {
      builder.and(reservation.status.eq(condition.getReservationStatus()));
    }
    ...
    if (condition.getLayerOne() != null) {
      builder.and(
          closure.depth.eq(1)
              .and(closure.assetClosureId.descendantId.eq(asset.id))
              .and(closure.assetClosureId.ancestorId.eq(
                  Long.parseLong(condition.getLayerOne())
              ))
      );
    }

    List<GetUserReservationResponseDto> content = query
        .select(Projections.constructor(
            GetUserReservationResponseDto.class,
            reservation.id,
            reservation.startAt,
            reservation.endAt,
            reservation.status,
