...
public class AssetQueryRepositoryImpl implements AssetQueryRepository {
    ...
    @Override
    public List<RawDescendantAssetResponseDto> searchDescendantsNoPaging(AssetSearchCondition condition) {
        ...
        if (condition.getKeyword() != null && !condition.getKeyword().isEmpty()) {
            builder.and(asset.name.containsIgnoreCase(condition.getKeyword())
                .or(asset.description.containsIgnoreCase(condition.getKeyword())));
        }
