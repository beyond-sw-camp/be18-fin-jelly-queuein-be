...
public class AppliedReservationsQueryAdapterImpl implements AppliedReservationsQueryAdapter {
  ...
  //TODO : dto 에 대해 string status 따로 service에서 추가해줘야함
  @Override
  public Page<GetAppliedReservationResponseDto> search(
    ...
    // === 조회 ===
    List<GetAppliedReservationResponseDto> content = query
        ...
        .from(reservation)
        .join(asset).on(asset.id.eq(reservation.assetId))
        .leftJoin(category).on(category.id.eq(asset.categoryId))
        .leftJoin(applicant).on(applicant.id.eq(reservation.applicantId))
        .leftJoin(respondent).on(respondent.id.eq(reservation.respondentId))
        .leftJoin(closure)
