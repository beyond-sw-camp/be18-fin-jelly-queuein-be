...
CREATE TABLE `asset` (
                         ...
                         `cost_per_hour`	DECIMAL(12, 3)	NOT NULL	DEFAULT 0.00	COMMENT '시간당 가상 청구 비용',
                         `period_cost`	DECIMAL(12, 3)	NOT NULL	DEFAULT 0.00	COMMENT '단위 기간(월, 주 등) 고정비',
                         `version`	BIGINT(20)	NOT NULL	COMMENT '낙관적 락 버전',
                         `created_at`	TIMESTAMP(6)	NOT NULL	DEFAULT CURRENT_TIMESTAMP	COMMENT '생성 시각',
                         `created_by`	BIGINT(20)	NOT NULL	COMMENT '생성자',
...
CREATE INDEX idx_asset_history_category_id ON asset_history(category_id); -- 필요 시
...
-- asset closure
CREATE INDEX idx_asset_closure_ancestor ON asset_closure(ancestor_id);
CREATE INDEX idx_asset_closure_descendant ON asset_closure(descendant_id);


-- unique
-- user_no
ALTER TABLE `user`
    ADD CONSTRAINT `uq_user_user_no` UNIQUE (`user_no`);
